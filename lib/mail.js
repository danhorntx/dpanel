...
function ensurePostfixMailbox(email) {
  ...
  // execSync runs through /bin/sh -c, but {cur,new,tmp} brace expansion is
  // bash-only — under dash it's a literal directory name. Spell out the dirs
  // to avoid creating a directory called "{cur,new,tmp}" on Debian/Ubuntu.
  const maildirPath = path.join(VMAIL_BASE, domain, user, 'Maildir');
  run(`mkdir -p ${maildirPath}/cur ${maildirPath}/new ${maildirPath}/tmp`, 'mail:mkdir', email);
...
async function addAccount(email, password, quota) {
  ...
  // We leave uid/gid/home empty — Dovecot's userdb override_fields supplies
  // those (uid=vmail gid=vmail home=/var/mail/vhosts/%d/%n). Quota lives in
  // the extra_fields block as userdb_quota_rule.
  const q = quota || '1G';
  const entry = `${email}:${hash.trim()}::::::userdb_quota_rule=*:bytes=${q}\n`;
