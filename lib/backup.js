...
// ── Matomo analytics DB backup ────────────────────────────────────────────────
// Matomo runs as a docker stack at /opt/matomo with its own MariaDB container
// ("matomo-db") — the host mysql client and `mysql.dumpDatabase()` can't reach
// it. Shell out to `docker exec` instead. Output filename mirrors the
// "<group>_db_<ts>.sql.gz" pattern so cleanup() picks it up under the
// "matomo" group and keeps the last N like any other domain.
async function backupMatomo() {
  ensureDir();
  const composeDir = '/opt/matomo';
  if (!fs.existsSync(`${composeDir}/.env`)) {
    throw new Error('Matomo .env not found — is the stack installed?');
  }
  // Pull the app-level DB password out of the .env. Root pw is also there but
  // mariadb-dump only needs the app user, which has full access to its DB.
  const envText = fs.readFileSync(`${composeDir}/.env`, 'utf8');
  const pw = envText.split('\n').find(l => l.startsWith('MATOMO_DB_PW='))?.slice('MATOMO_DB_PW='.length).trim();
  if (!pw) throw new Error('MATOMO_DB_PW missing from /opt/matomo/.env');

  const ts   = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
  const file = `matomo_db_${ts}.sql.gz`;
  const out  = path.join(BACKUP_DIR, file);

  // mariadb-dump → gzip → file. Stdin/stdout pipeline; password via stdin to
  // avoid leaking on argv (visible in /proc to other users on the host).
  await runAsync(
    `docker exec -i matomo-db mariadb-dump --single-transaction --quick --routines --triggers --events ` +
    `-u matomo -p${shellEscape(pw)} matomo | gzip > ${out}`,
    'backup:matomo', 'matomo'
  );
  logAction('backup:matomo', 'matomo', file);
  return { file, path: out };
}

function shellEscape(s) {
  // Single-quote escape for use inside double-quoted-by-default shell context.
...
module.exports = { list, backupFiles, backupDatabase, backupAll, backupMatomo, restoreFiles, restoreDatabase, deleteBackup, cleanup, BACKUP_DIR };
