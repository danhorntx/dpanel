...
function webmailVhostBody(subdomain) {
  return `    ServerName ${subdomain}
    ...
    #
    # IMPORTANT: the root path "/" must map to /webmail.html (the actual
    # classic webmail page) — NOT to DPanel root, which is the admin login
    # form. Subpaths (CSS/JS/api fetches issued by the classic page) preserve
    # their path so /css/base.css, /api/mail/..., etc. still resolve.
    SSLProxyEngine On
    SSLProxyVerify none
    ...
    RewriteEngine On
    # Root → classic webmail page.
    RewriteCond %{HTTP_COOKIE} (^|;\\s*)webmail_mode=classic
    RewriteCond %{REQUEST_URI} ^/?$
    RewriteRule ^ https://127.0.0.1:8080/webmail.html [P,L]
    # Everything else (assets, /api/mail/*, etc.) preserves its path.
    RewriteCond %{HTTP_COOKIE} (^|;\\s*)webmail_mode=classic
