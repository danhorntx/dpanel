...
// ── Access-group mutual exclusivity ────────────────────────────────────────
// Each DPanel-managed user belongs to EXACTLY ONE of these access groups.
// Mixing them produces broken behavior because OpenSSH's "first match wins"
// rule means the most restrictive block can override settings unexpectedly
// (e.g. a user in both dpanel-sftp and dpanel-shell gets jailed because
// dpanel-sftp's ChrootDirectory + ForceCommand come first in the drop-in).
const ACCESS_GROUPS = ['dpanel-sftp', 'dpanel-ftp', 'dpanel-shell'];

/**
 * Idempotently set a user's access-group membership. `target` is one of:
 *
 *   'sftp'  → dpanel-sftp  (chrooted SFTP, key-only — new SSH-first domains
 *                            whose docroot is /home/<user>/public_html)
 *   'ftp'   → dpanel-ftp   (no chroot, SFTP-only, password + key — legacy /
 *                            adopted domains with /var/www/<domain> docroot,
 *                            and any explicit FTP-fallback case)
 *   'shell' → dpanel-shell (no chroot, key-only, full bash shell)
 *
 * Removes the user from every other access group before adding the target.
 * www-data and the user's own gid group are not touched.
 */
function setAccessGroup(username, target) {
  const groupName = `dpanel-${target}`;
  if (!ACCESS_GROUPS.includes(groupName)) {
    throw new Error(`setAccessGroup: unknown target '${target}'. Use sftp/ftp/shell.`);
  }
  for (const g of ACCESS_GROUPS) {
    if (g === groupName) continue;
    try { execSync(`gpasswd -d ${username} ${g}`, { stdio: 'pipe' }); }
    catch (_) { /* user wasn't in that group — harmless */ }
  }
  try { execSync(`usermod -aG ${groupName} ${username}`); }
  catch (err) { throw new Error(`usermod failed adding ${username} to ${groupName}: ${err.message}`); }
}
  ...
  if (!fs.existsSync(MIGRATION_FLAG)) {
    try {
      ...
      for (const a of accounts) {
        ...
        // Pick the access group based on the account's CURRENT state, not
        // accumulate. Anyone without a key → ftp (password-allowed, no
        // chroot — and most legacy users have a docroot outside /home).
        // Anyone with a key → sftp by default; allowShell promotes to shell.
        const target = a.allowShell ? 'shell'
                     : a.hasSshKey  ? 'sftp'
                                    : 'ftp';
        try { setAccessGroup(a.username, target); } catch (_) {}
      }
...
function createAccount({ domain, username, password, docRoot, allowShell = false, sshKey = '' }) {
  ...
  // Legacy mode: docroot lives outside /home, so we want NO chroot. Start
  // the user in www-data only; setAccessGroup below assigns exactly one
  // access group. Shell wins over FTP — both work, but shell is the user's
  // stated intent if allowShell=true.
  execSync(`useradd -d ${sanitizePath(home)} -s ${shell} -M -G www-data ${username}`);
  setAccessGroup(username, allowShell ? 'shell' : 'ftp');
...
function provisionDomainUser(spec) {
  ...
  // Single access group, chosen by precedence:
  //   shell > ftp > sftp
  // shell wins because if the operator asked for shell, chrooting the SFTP
  // side would be inconsistent. ftp wins over sftp when allowFtp is on
  // because password auth needs the dpanel-ftp Match block (which is also
  // un-chrooted, which is fine for new SSH-first users — Unix perms still
  // protect /home/<other-user>).
  const accessGroup = allowShell ? 'shell' : allowFtp ? 'ftp' : 'sftp';

  // useradd. -m creates /home/<user>; we then chown root:root + chmod 755
  // to satisfy the chroot requirement when the user lands in dpanel-sftp.
  // The user owns public_html/ inside.
  execSync(`useradd -m -d ${home} -s ${shell} -G www-data ${username}`);
  setAccessGroup(username, accessGroup);
...
async function enableFtp(domain, password, opts = {}) {
  ...
  const [acct] = await pool.query(
    'SELECT username, allow_shell FROM dpanel_sftp_accounts WHERE domain = ? LIMIT 1', [domain]
  );
  if (!acct.length) throw new Error(`No deploy user exists for ${domain}.`);
  const username = acct[0].username;

  // Shell users keep dpanel-shell (it accepts both key + interactive sftp
  // via the shell). Non-shell users move to dpanel-ftp (password + SFTP,
  // no chroot). Either way: single-group invariant maintained.
  setAccessGroup(username, acct[0].allow_shell ? 'shell' : 'ftp');
  setPassword(username, password);
...
async function disableFtp(domain, opts = {}) {
  const [acct] = await pool.query(
    'SELECT username, allow_shell, doc_root FROM dpanel_sftp_accounts WHERE domain = ? LIMIT 1', [domain]
  );
  if (!acct.length) return;
  const username   = acct[0].username;
  const allowShell = !!acct[0].allow_shell;
  const docRoot    = acct[0].doc_root || '';

  // Pick the right resting group when FTP gets turned off:
  //   shell user  → stay in dpanel-shell
  //   chrootable  → dpanel-sftp (docroot inside /home/<user>)
  //   legacy      → stay in dpanel-ftp because their docroot is outside
  //                 /home and chrooting to /home/<user> would jail them
  //                 away from their files (the bug we just hit).
  const chrootable = docRoot.startsWith(`${HOME_BASE}/${username}/`);
  const target     = allowShell  ? 'shell'
                   : chrootable  ? 'sftp'
                                 : 'ftp';
  setAccessGroup(username, target);
  if (target !== 'ftp') lockPassword(username);
