...
// ── Read bare base64 public key (no DKIM header) ─────────────────────────────
// Returns just the base64 key string, e.g. "MIIBIjANBgkq..."
// Use this whenever you need to write a DKIM TXT record to DNS.
function getBareKey(domain) {
  sanitizeDomain(domain);
  ...
  // Concatenate all quoted strings, then extract everything after p=
  const chunks = [];
  const re = /"([^"]+)"/g;
  let m;
  while ((m = re.exec(content)) !== null) chunks.push(m[1]);
  const full = chunks.join('');
  const pm = full.match(/p=([A-Za-z0-9+/=]+)/);
  return pm ? pm[1] : null;
}

// ── Read public key as full TXT record value (for display only) ───────────────
// Returns "v=DKIM1; k=rsa; p=MIIBI..."  — do NOT write this directly to DNS,
// use getBareKey() + dns.addDkimRecord() which handles chunking.
function getPublicKey(domain) {
  const bare = getBareKey(domain);
  return bare ? `v=DKIM1; h=sha256; k=rsa; p=${bare}` : null;
}
...
module.exports = { generateKey, getBareKey, getPublicKey, hasDkim, removeKey, getDnsRecords, verifyDns };
