...
// ── issueWebrootCert — certonly --webroot, for hostnames that don't have an Apache vhost ──
// Used for mail.<domain> (consumed by Dovecot/Postfix), where we don't want to
// create an Apache vhost just to satisfy ACME. The default Apache vhost serves
// /var/www/html which is enough for the ACME challenge file.
async function issueWebrootCert(domain, email, webroot = '/var/www/html') {
  sanitizeDomain(domain);
  sanitizePath(webroot);
  const safeEmail = (email || `admin@${domain}`).replace(/[^a-zA-Z0-9@._+-]/g, '');
  return runAsync(
    `certbot certonly --webroot -w ${webroot} -d ${domain} --non-interactive --agree-tos --email ${safeEmail} --expand`,
    'ssl:webroot', domain
  );
}
...
module.exports = { listCerts, requestCert, autoSSL, issueWebrootCert, renewCert, renewAll, reissueCert, revokeCert };
