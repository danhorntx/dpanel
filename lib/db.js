...
async function migrate() {
  ...
  try {
    ...
    // ── Add Domain SSH-first rewrite (2026-05) ─────────────────────────────────
    //
    // dpanel_sftp_accounts is reused but acquires a password_disabled flag —
    // SSH-first domains land with password auth turned off (key-only). The
    // existing has_ssh_key column stays for backwards display compat.
    await conn.query(`ALTER TABLE dpanel_sftp_accounts ADD COLUMN IF NOT EXISTS password_disabled TINYINT(1) NOT NULL DEFAULT 0`).catch(() => {});
    await conn.query(`ALTER TABLE dpanel_sftp_accounts ADD COLUMN IF NOT EXISTS ftp_enabled       TINYINT(1) NOT NULL DEFAULT 0`).catch(() => {});
    await conn.query(`ALTER TABLE dpanel_sftp_accounts ADD COLUMN IF NOT EXISTS chroot_dir        VARCHAR(512) DEFAULT NULL`).catch(() => {});

    // Per-domain authorized SSH public keys. Many keys per domain user.
    // Fingerprint is sha256 of the public key blob (matches `ssh-keygen -lf`).
    // Source: 'pasted' (user pasted a public key — most common) or 'generated'
    //         (we generated the keypair in a tmpfs and showed the private once).
    await conn.query(`
      CREATE TABLE IF NOT EXISTS dpanel_domain_keys (
        id           INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
        domain       VARCHAR(253) NOT NULL,
        ...
        source       ENUM('pasted','generated') NOT NULL DEFAULT 'pasted',
        created_by   INT UNSIGNED,
        created_at   DATETIME DEFAULT CURRENT_TIMESTAMP,
        last_used_at DATETIME,
        UNIQUE KEY uk_domain_fp (domain, fingerprint),
        INDEX (domain),
        FOREIGN KEY (created_by) REFERENCES dpanel_users(id) ON DELETE SET NULL
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
    `);

    // Per-domain feature flags. One row per domain.
    //   placeholder_enabled — Coming Soon static page served instead of docroot
    //   ssl_retry_state     — 'pending' | 'active' | 'failed' (nullable until first try)
    //   ssl_last_attempt_at — most recent attempt (any host); convenience for UI
    await conn.query(`
      CREATE TABLE IF NOT EXISTS dpanel_domain_settings (
        domain              VARCHAR(253) NOT NULL PRIMARY KEY,
        placeholder_enabled TINYINT(1)   NOT NULL DEFAULT 0,
        ssl_retry_state     ENUM('pending','active','failed') DEFAULT NULL,
        ssl_last_attempt_at DATETIME     DEFAULT NULL,
        notes               TEXT,
        created_at          DATETIME DEFAULT CURRENT_TIMESTAMP,
        updated_at          DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
    ...
    // SSL issuance attempt log. One row per certbot attempt — both initial
    // provisioning and the scheduled retries (H+1, H+5, H+13, H+25). The
    // retry cron queries this table to decide what's still pending and
    // whether the budget has been exhausted.
    // Matomo site mapping. One row per managed domain that we've registered
    // with the Matomo install at analytics.danhorntx.com. matomo_site_id is
    // the integer the Matomo tracker URL needs. Lives on dpanel_domain_meta
    // (created above) rather than its own table because there's at most one
    // matomo siteId per domain and we already key by domain there.
    await conn.query(`ALTER TABLE dpanel_domain_meta ADD COLUMN IF NOT EXISTS matomo_site_id INT UNSIGNED DEFAULT NULL`).catch(() => {});

    await conn.query(`
      CREATE TABLE IF NOT EXISTS dpanel_ssl_attempts (
        id         BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
        domain     VARCHAR(253) NOT NULL,
        ...
        INDEX (domain, host, attempted_at),
