...
window.usersMgr = (() => {
  ...
  async function loadAuditLog() {
    ...
    const filter = document.getElementById('auditLogFilter')?.value.trim() || '';
    const params = new URLSearchParams({ limit: '200' });
    if (filter) params.set('action', filter);

    const data = await api.get(`/api/users/audit-log?${params}`);
    if (!data?.success) {
      tbody.innerHTML = `<tr><td colspan="6"><div class="empty-state"><p class="text-red">${data?.error || 'Failed'}</p></div></td></tr>`;
      return;
    }
    if (!data.data.length) {
      tbody.innerHTML = `<tr><td colspan="6"><div class="empty-state"><p>No audit log entries found.</p></div></td></tr>`;
      return;
    }

    function actionBadge(action) {
      if (action.startsWith('user:')) return `<span class="badge badge-blue">${action}</span>`;
      if (action.startsWith('settings:')) return `<span class="badge badge-amber">${action}</span>`;
      if (action.includes('delete') || action.includes('revoke')) return `<span class="badge badge-red">${action}</span>`;
      return `<span class="badge badge-muted">${action}</span>`;
    }

    tbody.innerHTML = data.data.map(r => `
      <tr>
        <td class="text-sm text-secondary" style="white-space:nowrap">${new Date(r.created_at).toLocaleString()}</td>
        <td class="font-medium text-sm">${r.username || '<em class="text-muted">system</em>'}</td>
        <td>${actionBadge(r.action)}</td>
        <td class="text-sm text-secondary td-truncate" title="${r.target || ''}">${r.target || '—'}</td>
        <td class="text-sm text-secondary td-truncate" title="${r.detail || ''}">${r.detail || '—'}</td>
        <td class="td-mono text-sm text-muted">${r.ip || '—'}</td>
      </tr>`).join('');
  }

  return { load, openAdd, openEdit, save, confirmDelete, loadAuditLog };
})();
